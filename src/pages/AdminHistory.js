...
function AdminHistory() {
  ...
  // Fetch history records from Realtime Database
  useEffect(() => {
  ...
  const handleAddSave = async () => {
    if (!newRecord.team || !newRecord.house) {
      alert("Please select both a team and a dormitory.");
      return;
    }
    try {
      await push(ref(db, "history"), newRecord);
      setAdding(false);
    } catch (error) {
